...
===========================================================================
  ...
  Data_Program.cpp //based on neo/d3xp/script/Script_Program.cpp

    Created on: 21 de feb. 2020
      Author: Biel Bestué de Luna


    Open source under a GPLv3 License
...
#include "../../framework/scripting/Data_Program.h"
#include "../../d3xp/gamesys/SaveGame.h" //TODO needed?
...
only for scriptObjects, as those are the only ones that can inherit.
...
{
	if( type != ev_object ) // only scriptObjects can inherit
	{
		return false;
	}
	...
	for( idTypeDef* superType = auxType; superType != NULL; superType = superType->auxType )
	{
	    // test for the possiblity of this being already a 'super' to the
	    // ScriptObject we are triying to inherit from.
...
bool idTypeDef::MatchesType( const idTypeDef& matchtype ) const
{
	...
	for( int i = 0; i < matchtype.parmTypes.Num(); i++ )
	{
...
bool idTypeDef::MatchesVirtualFunction( const idTypeDef& matchfunc ) const
{
	...
	for( int i = 1; i < matchfunc.parmTypes.Num(); i++ )
	{
...
void idTypeDef::AddFunctionParm( idTypeDef* parmtype, const char* name )
{
	if( type != ev_function )
	{
        ...
        common->Error( "idTypeDef::AddFunctionParm : tried to add parameter on non-function type" );
#endif
	}
...
void idTypeDef::AddField( idTypeDef* fieldtype, const char* name )
{
	if( type != ev_object )
	{
		...
		common->Error( "idTypeDef::AddField : tried to add field to non-object type" );
#endif
	}
...
idTypeDef* idTypeDef::SuperClass() const
{
	if( type != ev_object )
	{
		...
		common->Error( "idTypeDef::SuperClass : tried to get superclass of a non-object type" );
#endif
	}
...
idTypeDef* idTypeDef::ReturnType() const
{
	if( type != ev_function )
	{
		...
		common->Error( "idTypeDef::ReturnType: tried to get return type on non-function type" );
#endif
	}
...
void idTypeDef::SetReturnType( idTypeDef* returntype )
{
	if( type != ev_function )
	{
		...
		common->Error( "idTypeDef::SetReturnType: tried to set return type on non-function type" );
#endif
	}
...
idTypeDef* idTypeDef::FieldType() const
{
	if( type != ev_field )
	{
		...
		common->Error( "idTypeDef::FieldType: tried to get field type on non-field type" );
#endif
	}
...
void idTypeDef::SetFieldType( idTypeDef* fieldtype )
{
	if( type != ev_field )
	{
		...
		common->Error( "idTypeDef::SetFieldType: tried to set return type on non-function type" );
#endif
	}
...
idTypeDef* idTypeDef::PointerType() const
{
	if( type != ev_pointer )
	{
		...
		common->Error( "idTypeDef::PointerType: tried to get pointer type on non-pointer" );
#endif
	}
...
void idTypeDef::SetPointerType( idTypeDef* pointertype )
{
	if( type != ev_pointer )
	{
		...
		common->Error( "idTypeDef::SetPointerType: tried to set type on non-pointer" );
#endif
	}
...
void idTypeDef::AddFunction( const function_t* func )
{
	...
	{
	    //test if the names of both functions are the same
		if( !idStr::Cmp( functions[ i ]->def->Name(), func->def->Name() ) )
		{
		    //test if the parameters and base function is the same
...
/*
============
idScriptObject::Clear
============
*/
void idScriptObject::Clear()
{
    data = NULL;
	type = &type_object;
	workingProgram = NULL;
...
void idScriptObject::Save( idSaveGame* savefile ) const
{
	...
	else
	{
		...
		int size = type->Size(); // RB: 64 bit fix, changed size_t to int
		savefile->WriteInt( size );
		savefile->Write( data, size );
		//savefile->WriteInt( workingProgram ); //TODO work out a the position in memory of the program
	}
}
...
void idScriptObject::Restore( idRestoreGame* savefile )
{
	...
	//savefile->ReadInt( workingProgram ); //TODO work out a the position in memory of the program
}

/*
============
idScriptObject::SetWorkingProgram

points to the workingProgram that the script object will be using for all it's functions
============
*/
void idScriptObject::SetWorkingProgram( idProgram p ) {
    workingProgram = &p;
}
...
bool idScriptObject::SetType( const char* typeName )
{
	...
	// lookup the type
	//newtype = gameLocal.program.FindType( typeName ); //we might no longer operate only on that program
	newtype = workingProgram->FindType( typeName );
	...
	if( newtype != type )
	{
		...
		if( !newtype )
		{
			...
			common->Warning( "idScriptObject::SetType: Unknown type '%s'", typeName );
			return false;
		}
		...
		if( !newtype->Inherits( &type_object ) )
		{
			...
			common->Warning( "idScriptObject::SetType: Can't create object of type '%s'.  Must be an object type.", newtype->Name() );
			return false;
		}
...
const function_t* idScriptObject::GetFunction( const char* name ) const
{
	...
	//func = gameLocal.program.FindFunction( name, type ); //we might no longer operate only on that program
	func = workingProgram->FindFunction( name, type );
	return func;
}
...
void idVarDef::SetValue( const eval_t& _value, bool constant )
{
	...
	switch( typeDef->Type() )
	{
		...
		default :
			...
			common->Error(  "weird type on '%s'", Name() );
#endif
			break;
	}
}
...
void idVarDef::PrintInfo( idFile* file, int instructionPointer ) const
{
	...
	switch( etype )
	{
		case ev_jumpoffset :
			...
			jumpst = &workingProgram->GetStatement( jumpto );
			//file->Printf( "address %d [%s(%d)]", jumpto, gameLocal.program.GetFilename( jumpst->file ), jumpst->linenumber );
			file->Printf( "address %d [%s(%d)]", jumpto, workingProgram->GetFilename( jumpst->file ), jumpst->linenumber );
			break;
...
/*
============
idVarDef::SetWorkingProgram

points to the workingProgram that the variable will be using.
============
*/
void idVarDef::SetWorkingProgram( idProgram p ) {
    workingProgram = &p;
}
...
idTypeDef* idProgram::GetType( idTypeDef& type, bool allocate )
{
	...
	*/
	idTypeDef* found = FindType( type.Name() ) {        //reuse code

	    if( found && found->MatchesType( type ) ) {
	        return found;
	    }
	}
...
/*
============
idProgram::ReserveDefMemory     // RB: moved from AllocDef
============
*/
byte* idProgram::ReserveDefMemory( int size )
{
	...
	if( numVariables > sizeof( variables ) )
	{
		...
		common->Error( "Exceeded global memory size (%" BFG_PRIiSIZE " bytes)", sizeof( variables ) );
#endif
	}
...
/*
============
idProgram::AllocVarDef     // RB: moved from AllocDef
============
*/
