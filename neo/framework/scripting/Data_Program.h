...
===========================================================================
  ...
  Data_Program.h //based on /neo/d3xp/script/Script_Program.h
...
#ifndef __DATA_PROGRAM_H__
#define __DATA_PROGRAM_H__
...
class idVarDefName;
class idTypeDef;
class idProgram; //TODO maybe is defined before?
class idThread;
class idSaveGame;
class idRestoreGame;
...
typedef enum
{
  ...
  ev_field,
  ev_function,
  ev_virtualfunction, //a funtion on a thread?
  ev_pointer,
  ev_object,
  ev_jumpoffset,
  ev_argsize,
...
private:
	idTypeDef*					type;
  idProgram*				workingProgram; //refernce to the working loaded script and data space that this script object makes use of
...
public:
  ...
  void            Clear();
	void						Free();
  void            SetWorkingProgram( idProgram p );
...
ID_INLINE void idScriptVariable<type, etype, returnType>::LinkTo( idScriptObject& obj, const char* name )
{
    ...
    Common->ScriptError( "Missing '%s' field in script object '%s'", name, obj.GetTypeName() ); // we no longer sit on GameLocal dependencies
	}
}
...
private:
  ...
  idProgram*		workingProgram; //refernce to the working loaded script and data space that this variable makes use of
